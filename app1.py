...
def send_money():
    ...
    transaction_id = transactions.send_money_by_user_id(sender_id, receiver_id, amount, message)
...
def request_money():
    ...
    transaction_id = transactions.request_money_by_user_id(sender_id, receiver_id, amount, message)
    transactions = Transactions.query.filter_by(id=transaction_id).first()
    if transactions is None:
        return failure_response("Could not create transaction")
    return success_response(transactions.serialize())
...
def accept_or_deny_payment(transaction_id):
    ...
    if accepted == "True":
        transaction_id = transactions.accept_transaction(transaction_id)
...
def register_account():
    ...
    if username is None or email is None or password is None:
        return json.dumps({"error": "Invalid username, email, or password"})
